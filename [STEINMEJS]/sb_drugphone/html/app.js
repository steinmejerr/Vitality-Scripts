...
const activeChatPreview=document.getElementById('active-chat-preview');
...
function getOfferKey(offer){
  ...
  return `${offer.productId}:${offer.amount}:${offer.total}:${offer.expiresAt||0}`;
}
...
function render(){
  ...
  activeChatEl.classList.toggle('hidden',!state.offer);
  if(state.offer){
    activeChatTitle.textContent=`Ukendt køber · ${state.offer.productLabel}`;
    activeChatPreview.textContent=state.offer.accepted
      ? 'Lokationen er modtaget – tryk for at se samtalen'
      : `${state.offer.amount}x ${state.offer.productLabel} · ${money(state.offer.total)}`;
  }

  if(currentView==='chat' && state.offer) showChat();
  else showHome();
}

function showHome(){currentView='home';homeView.classList.remove('hidden');chatView.classList.add('hidden')}
function showChat(){
  if(!state.offer){showHome();return}
  currentView='chat';
...
window.addEventListener('message',e=>{
  ...
  if(action==='sync'){
    const newOfferKey=getOfferKey(data.offer);
    const isNewOffer=newOfferKey && newOfferKey!==lastOfferKey;
    state=data;
    if(isNewOffer) currentView='chat';
    if(!newOfferKey) currentView='home';
